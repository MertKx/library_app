        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
            ...
            <div class="flex gap-2 items-center">
                @auth
                    ...
                    <a href="{{ route('bulk-import.index') }}" class="bg-zinc-700 hover:bg-zinc-600 text-white px-4 py-2 rounded shadow transition flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                    ...
                    <a href="{{ route('bulk-import-history.index') }}" class="bg-zinc-700 hover:bg-zinc-600 text-white px-4 py-2 rounded shadow transition flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12h15M12 4.5v15" />
                        </svg>
                        Follow Imports
        ...
        @if($books->count())
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                @foreach($books as $book)
                    <div class="bg-white dark:bg-zinc-900 rounded-lg shadow p-4 flex flex-col h-96">
                        ...
                        <div class="flex-1 flex flex-col gap-1 min-h-0">
                            <h2 class="text-lg font-semibold text-zinc-900 dark:text-white line-clamp-2">{{ $book->book_name }}</h2>
                        ...
                        <div class="mt-4 flex gap-2 mt-auto">
                            ...
                            @auth
                                <a href="{{ route('books.edit', $book) }}" class="flex-1 bg-zinc-700 hover:bg-zinc-600 text-white px-3 py-1 rounded text-center transition">Edit</a>
            ...
            <div class="text-center text-zinc-500 dark:text-zinc-400 py-12">
                ...
                @else
                    <div class="space-y-4">
                        ...
                        <a href="{{ route('login') }}" class="inline-block bg-zinc-800 hover:bg-zinc-700 text-white px-6 py-2 rounded shadow transition">
                            Login to Add Books
                        </a>
                    </div>
                @endauth
